...
async def fetch_tg_photo_ref(bot, user_id: int) -> str | None:
    """
    Запрашивает последний аватар Telegram-пользователя.

    Возвращает token-less ссылку вида ``tg:<file_path>`` (БЕЗ токена бота) или
    None при ошибке. Токен НЕ включаем в сохраняемое значение: иначе он попал бы
    в БД, бэкапы и HTML-страницы. Реальный файл отдаёт защищённый прокси
    (/admin/avatar/...), подставляя токен на сервере.
    """
    try:
