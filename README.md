...
- **TRUSTED_PROXIES** — кому доверять заголовок `X-Forwarded-For`
  (`*` на Railway/Render, где адрес прокси динамический). Пусто — заголовок
  игнорируется: подделать свой адрес нельзя, но за прокси все посетители
  выглядят одинаково и лимит запросов становится общим.
